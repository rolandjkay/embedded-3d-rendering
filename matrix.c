...
{
  Vector u, v, _up_vector, _look_vector;

  _up_vector.x = FIX_TO_FLT(up_vector->x);
  _up_vector.y = FIX_TO_FLT(up_vector->y);
  _up_vector.z = FIX_TO_FLT(up_vector->z);

  _look_vector.x = FIX_TO_FLT(look_vector->x);
  _look_vector.y = FIX_TO_FLT(look_vector->y);
  _look_vector.z = FIX_TO_FLT(look_vector->z);

  //vector_subtract(&n, look_point, camera_location);
  //vector_normalize(&n);
  vector_cross_product(&u, &_up_vector, &_look_vector);
  vector_cross_product(&v, &_look_vector, &u);

  matrix_tf_change_of_basis(dst, &u, &v, &_look_vector);
}
